...
namespace GasSizing {
    class Program {
        static void Main (string[] args) {
            #region Data Input
            double Qs = 3397; //Flow rate unit:m3/h
            double W = 56700; //125000;//Qs * .04475 * 44.01; //Mass flow rate W=Qs*N8/N9*M unit:Kg/h

            double P1 = 3548; //Inlet Pressure unit:kPa;
            double P2 = 1825; //Outlet pressure unit:kPa;
            double T1 = 533; //Inlet temperature, unit:K;
            double D1 = 150; //Inlet pipe size, unit:mm
            double D2 = 150; //Outlet pipe size, unit:mm

            string Fluid = "Steam"; //fluid type, incompressible or compressible
            //double Density = 8.389; //Fluid density, unit kg/m3
            //double DensitySTD = 1978; //fluid density at standard condition unit kg/m3

            double Z1;////Compressibility, unit kPa;

            if (Fluid == "Steam") {
                double Zs1 = 1 - 0.024 * Math.Pow (P1 / 100, 0.654) / Math.Pow (220 - P1 / 100, 0.08);
                Console.WriteLine ("ZS1 = " + Zs1);
                double Density = 216.49 * P1 / 100 / Zs1 / T1; //Fluid density, unit kg/m3
                Console.WriteLine ("Density =" + Density );
                //double DensitySTD = 1978; //fluid density at standard condition unit kg/m3
                Z1 = Zs1; //Compressibility, unit kPa;

            } else {

                 Z1 = .991; //Compressibility, unit kPa;
            }
            ...
            double d = 100; //Valve size unit:mm
            string Class;
